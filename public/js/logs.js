...
tabBtns.forEach((btn) => {
    btn.addEventListener("click", () => {
        ...
        filterClub.placeholder =
            activeTab === "club" ? "Filter by club name..." : "Filter by member name...";
        filterClub.value = "";
...
function renderLogs(logs) {
    ...
    `
        ...
        const namedLogs = logs.filter((l) => l.memberName && l.memberName.trim() !== "");
        if (namedLogs.length === 0) {
            list.innerHTML = `<p class="empty">No member reviews yet. Be the first to add one!</p>`;
            return;
        }
        list.innerHTML = `<div class="logs-grid">${namedLogs.map((l) => `
