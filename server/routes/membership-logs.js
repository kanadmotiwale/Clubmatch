...
router.post("/", async (req, res) => {
    try {
        const db = getDB();
        const { clubName, weeklyHours, benefits, challenges } = req.body;
...
router.get("/stats/:clubName", async (req, res) => {
    try {
        const db = getDB();
        const result = await db.collection("membership_logs").aggregate([
            { $match: { clubName: req.params.clubName } },
            { $group: { _id: "$clubName", avgHours: { $avg: "$weeklyHours" } } },
        ]).toArray();
        if (result.length === 0) return res.status(404).json({ error: "No logs found for this club" });
        res.json({ clubName: req.params.clubName, avgHours: result[0].avgHours.toFixed(1) });
    } catch (err) {
...
router.get("/:id", async (req, res) => {
    try {
        const db = getDB();
        const log = await db.collection("membership_logs").findOne({ _id: new ObjectId(req.params.id) });
...
router.put("/:id", async (req, res) => {
    try {
        ...
        const result = await db.collection("membership_logs").findOneAndUpdate(
...
router.delete("/:id", async (req, res) => {
    try {
        const db = getDB();
        const result = await db.collection("membership_logs").deleteOne({ _id: new ObjectId(req.params.id) });
